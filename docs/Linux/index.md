# Linux相关博客

[Centos8安装FFmpeg](docs/Linux/Centos8-Install-FFmpeg/index.md)

[如何在Linux主机上定时运行某条命令或者程序以及注意事项](docs/Linux/run-commend-on-linux-Timming/index.md)

[关闭ssh的严格模式](docs/Linux/disable-strict-mode-for-ssh-connect/index.md)

[给服务器添加ssh公钥(免密登录)](./add-public-key-for-liuux-server.md)

[Linux服务器一键换源](./Linux-change-mirror-one-click.md)

[Centos使用](./centos-usage.md)

[自建rustdesk服务端](./rustdesk.md)

[linux服务器迁移](./linux-server-move-place.md)

[nginx相关](./nginx/index.md)

[wsl安装code-server](./wsl-install-code-server.md)

[Ubuntu20.04编译安装Openssl](./Ubuntu20.04-complie-and-install-Openssl.md)

[Frp使用心得](./frp-usage.md)

[记录服务器升级失败](./fail-to-update-cloud-based-ubuntu.md)

[快速部署frp](./deploy-frp-quickly.md)